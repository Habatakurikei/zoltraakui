# 作業内容
**`要件定義書`を元に、以下の指示に従ってPythonでディレクトリ構成を構築するコードを記述してください。コードのみ出力してください。**

# モジュール情報
以下出力pythonファイル冒頭に記載すること
from zoltraak.llms.gemini import `generate_response`
...
2. ディレクトリ構成は `ルートディレクトリ` の配下にすべて作成すること、README.md を除いて、それ以外のファイルは記述
    ...
    `要件定義ファイル`の中に`ファイル・フォルダ構成`がある場合、`ファイル・フォルダ構成`に忠実にディレクトリを作成すること
    その後、ディレクトリ名をリストに記述し実行
    2.3 osモジュールの機能を使用して、必要なファイルを作成する（ディレクトリの中に何もない場合は新しく作る）
    `要件定義ファイル`の中に`ファイル・フォルダ構成`がある場合、`ファイル・フォルダ構成`に忠実にファイルを作成すること
    以下参考、ただしメインファイルはプログラムを詳細に記載する
    ```
    files = [
    ('dirname', 'filename', 'description について中身を記述して下さい。「コードだけ出力して解説を含めないこと」を末尾に追加'),
    ]
    ```
    2.4 全てのpromptに`要件定義ファイル`を読み込みモードで取得した内容を入れて、ファイル名と必要なプロンプトを記載
